...
def confirmation_reservation(request): 
    ...
    if 'date_reservation' in reservation: 
        reservation['date_reservation'] = datetime.strptime( 
            reservation['date_reservation'], "%Y-%m-%d" 
        ).date() 
    if 'heure_reservation' in reservation: 
        reservation['heure_reservation'] = datetime.strptime( 
 
 
            reservation['heure_reservation'], "%H:%M:%S" 
        ).time() 
